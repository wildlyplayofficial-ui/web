...
async function handleUpdate(update: TgUpdate): Promise<void> {
  ...
  if (update.callback_query?.game_short_name) {
    ...
    // Pass message context so the client can attach game scores to this card
    // (chat+msg for group sendGame cards, imid for inline cards).
    const gameUrl = `${TMA_URL}?game=1&uid=${from.id}&name=${encodeURIComponent(from.first_name || "Player")}&chat=${chatId}${msgId ? `&msg=${msgId}` : ""}${imid ? `&imid=${encodeURIComponent(imid)}` : ""}`;
