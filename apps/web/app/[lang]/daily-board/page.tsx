...
export default async function DailyBoard({ params }: Props) {
  const lang = resolveLang((await params).lang);
  const dict = getDict(lang);
  const [allPicks, settledPicks, watching, noPlays, curatorRecord] = await Promise.all([
    getTodaysPicks(),
    getSettledPicks(),
    getActiveWatching(),
    getTodaysNoPlays(),
  ...
  // §7.1: curator picks only — khối Trợ lý AI đã gỡ (Peter duyệt 15/9/2026).
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-5 overflow-x-hidden">
      ...
      {/* 4. Khối Trợ lý AI (Scout) đã gỡ — Peter duyệt 15/9/2026. */}
