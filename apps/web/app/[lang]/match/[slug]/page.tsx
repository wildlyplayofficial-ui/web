    ...
    <article className="mx-auto max-w-[720px] px-5 py-12">
      ...
      {match.picks.length > 0 && ["won", "lost", "push"].includes(match.picks[0].status) && (
        <section className="mt-6 rounded-card border border-line bg-card p-4 shadow-card">
          ...
          ) : (<p className="mt-2 text-sm text-muted italic">{lang === "vi" ? (match.picks[0].author === "scout" ? "Đang chờ Trợ lý AI tổng kết nhận định này." : "Đang chờ Chú Tám Banh tổng kết nhận định này.") : match.picks[0].author === "scout" ? "Review pending \u2014 the Scout will review this play." : "Review pending \u2014 the Curator will review this play."}</p>)}
        </section>
      )}
