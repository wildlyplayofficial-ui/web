...
/** If the slug contains an aliased team name, return the canonical slug. */
...
export default async function MatchPage({ params, searchParams }: Props) {
  ...
  // 308 redirect alias slugs to canonical (e.g. czech-republic → czechia)
  ...
  // No DB content yet — show empty match page instead of 404
  if (!match) {
    ...
    const homeName = deslug(homePart);
    const awayName = deslug(awayPart);
      ...
      <article className="mx-auto max-w-[720px] px-5 py-12">
        ...
        <header className="mt-6">
          ...
          <h1 className="mt-3 font-display text-3xl font-bold leading-tight md:text-4xl">
            {hf && <span className="mr-1.5">{hf}</span>}{homeName}
            <span className="mx-2 text-muted">vs</span>
            {af && <span className="mr-1.5">{af}</span>}{awayName}
          </h1>
        </header>
        <div className="mt-8 rounded-card border border-line bg-card px-6 py-12 text-center">
          <p className="text-muted">{dict.match.noContent}</p>
        </div>
      </article>
    );
  }
  ...
  // Schema is built from DB fields only (team names, dates, league) — no user HTML.
  // Escape < to prevent script breakout, same pattern as news/[slug]/page.tsx.
  const schema = JSON.stringify(buildSportsEventSchema(match)).replace(/</g, "\\u003c");

  // Deduplicate posts by slug, prefer viewer's language
  ...
  return (
    <article className="mx-auto max-w-[720px] px-5 py-12">
      {/* Schema: server-generated from DB fields, escaped against injection */}
      <script
      ...
      <header className="mt-6">
        <p className="text-sm text-muted">
          {match.league}{match.league ? " \u00b7 " : ""}{formatKickoff(match.kickoffUtc, lang)}
        </p>
        <h1 className="mt-3 font-display text-3xl font-bold leading-tight md:text-4xl">
          ...
          return (
            <div className="mt-3 flex items-center gap-3">
              <span className="font-display text-2xl font-bold text-ink">
                {pick.home_score}&ndash;{pick.away_score}
              </span>
              {isSettled && (
                <span className={`rounded-md px-2 py-0.5 text-xs font-bold ${
                  pick.status === "won" ? "bg-brand/10 text-brand" :
                  pick.status === "lost" ? "bg-loss/10 text-loss" :
                  "bg-muted/10 text-muted"
                }`}>
                  {pick.status.toUpperCase()}
                  {pick.units_pl !== null && ` ${pick.units_pl > 0 ? "+" : ""}${pick.units_pl}u`}
                </span>
              )}
              {pick.odds_close !== null && (
                <span className="text-xs text-muted">
                  CLV: {pick.odds_publish} → {pick.odds_close}
                </span>
              )}
            </div>
          );
      ...
      {/* Curator's Pick = HERO (Nick: pick is the hero of the match page) */}
      {match.picks.length > 0 && (
        <section className="mt-8">
          <h2 className="mb-3 font-display text-lg font-bold">{dict.match.curatorPick}</h2>
      ...
      {/* Live Commentary — right below pick */}
      {match.picks.length > 0 && match.picks[0].fixture_id > 0 && (
        <MatchCommentary
          fixtureId={match.picks[0].fixture_id}
      ...
      {match.picks.length > 0 && ["won", "lost", "push"].includes(match.picks[0].status) && (
        <section className="mt-6 rounded-card border border-line bg-card p-4 shadow-card">
          ...
          {postmortemArticle ? (
            <>
              <p className="mt-2 text-sm text-ink">{postmortemArticle.meta_description}</p>
              {match.picks[0].status === "lost" && (match.picks[0] as unknown as { loss_type?: string }).loss_type && (
                <p className="mt-2 text-xs text-muted">
                  Loss type: <span className="font-medium text-ink">{(match.picks[0] as unknown as { loss_type: string }).loss_type.replace(/-/g, " ")}</span>
                </p>
              )}
              <Link
                href={withLang(`/news/${postmortemArticle.slug}`, lang)}
                className="mt-3 inline-block text-sm font-semibold text-brand transition-colors hover:text-ink"
              >
                {dict.match.readReview} &rarr;
              </Link>
            </>
          ) : (match.picks[0] as unknown as { postmortem_approved?: string }).postmortem_approved ? (
            <>
              <p className="mt-2 text-sm text-ink whitespace-pre-line">{(match.picks[0] as unknown as { postmortem_approved: string }).postmortem_approved.replace(/\*\*/g, "").replace(/\*/g, "")}</p>
              {match.picks[0].status === "lost" && (match.picks[0] as unknown as { loss_type?: string }).loss_type && (
                <p className="mt-2 text-xs text-muted">
                  Loss type: <span className="font-medium text-ink">{(match.picks[0] as unknown as { loss_type: string }).loss_type.replace(/-/g, " ")}</span>
                </p>
            ...
            <p className="mt-2 text-sm text-muted italic">Review pending — the Curator will review this play.</p>
          )}
        </section>
      )}

      {/* Curator's Watch (below commentary when pick exists) */}
      {match.watching && (
        <section className="mt-8">
          <h2 className="mb-3 font-display text-lg font-bold">{dict.match.curatorWatch}</h2>
          <WatchingTeaser items={[match.watching]} lang={lang} hideLinks />
        </section>
      )}
      ...
      {uniquePosts.length > 0 && (
        <section className="mt-8">
          <h2 className="mb-3 font-display text-lg font-bold">{dict.match.articles}</h2>
          <ul className="flex flex-col gap-3">
            {uniquePosts.map((post) => (
              <li key={post.id} className="rounded-card border border-line bg-card p-4 shadow-card">
                <span className="mb-1 inline-block rounded-md bg-card px-2 py-0.5 text-xs font-semibold uppercase text-muted">
                  {post.type}
                </span>
                ...
                <Link
                  href={withLang(`/news/${post.slug}`, lang)}
                  className="mt-1 inline-block text-sm font-semibold text-brand transition-colors hover:text-ink"
                >
                  {dict.match.readArticle} &rarr;
                </Link>
              </li>
            ))}
          </ul>
      ...
      {/* Empty state */}
      {!match.watching && match.picks.length === 0 && uniquePosts.length === 0 && (
        <div className="mt-8 rounded-card border border-line bg-card px-6 py-12 text-center">
          <p className="text-muted">{dict.match.noContent}</p>
        </div>
      )}
