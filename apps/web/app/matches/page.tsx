...
/** Known slug → display name overrides (slugify is lossy for abbreviations/special chars). */
...
/** Canonical name for dedup — merges known aliases (Turkey/Turkiye, Czechia/Czech Republic, etc.) */
...
export default async function MatchesIndex({ searchParams }: Props) {
  ...
  // Parse, deduplicate by display names + date, sort newest first
  ...
  // Dedup: same canonical name pair + date (±1 day for timezone overlap) = same match
  ...
  const matches = parsed.filter((m) => {
    ...
    // Also check ±1 day for timezone slug overlap (same match, different UTC date)
