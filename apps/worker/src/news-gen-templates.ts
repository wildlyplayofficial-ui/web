...
export interface PreviewData {
  ...
  pickUrl: string | null;
  /** "The Curator" or "The Scout" — firewall: must match pick author, never hardcode */
...
export function renderPreview(lang: NewsLang, data: PreviewData): Rendered {
  ...
  if (d.pickUrl && d.pickAuthor) lines.push(t.pick(d.pickAuthor, d.pickUrl));
  lines.push(t.outro);
...
const RESULT = {
  en: {
    ...
    intro: (d: ResultData) => `${d.home} ${d.homeScore}-${d.awayScore} ${d.away} — full-time in the ${d.competition}, ${d.dateUtc}.`,
    win: (winner: string) => `${winner} took the win.`,
    draw: 'The match ended in a draw.',
    pick: (author: string, url: string) => `${author} published a pick for this match: ${url}`,
  },
  vi: {
  ...
  th: {
    ...
    intro: (d: ResultData) => `${d.home} ${d.homeScore}-${d.awayScore} ${d.away} — จบเกมในศึก ${d.competition} วันที่ ${d.dateUtc}`,
  ...
  es: {
    ...
    intro: (d: ResultData) => `${d.home} ${d.homeScore}-${d.awayScore} ${d.away} — final del partido de ${d.competition}, ${d.dateUtc}.`,
