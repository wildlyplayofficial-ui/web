"""Hero tin SAU TRẬN 2400x1260: cartoon cầu thủ + 2 cờ/logo + TỈ SỐ to + dòng người ghi bàn.

Vì sao có tệp này: khuôn cũ `compose_flags_hero.py` chưa có tấm nền nên chắc chắn rớt
do-thumbnail (giống hệt `compose_team_hero.py` rớt 64,2%/78,7% chiều 26/8). Nền + tấm chữ
bê nguyên `compose_transfer.py` bản 2. Khác: hàng trên là CỜ A - TỈ SỐ - CỜ B thay cho mũi tên.

⚠️ Tỉ số là con số người đọc tin nhất trên ảnh — chỉ điền khi đã có 2 nguồn khớp.
   Trận chưa đá xong thì KHÔNG dùng khuôn này.

usage: python compose_result.py <cartoon.png> <co_A.png> <co_B.png> "<TỈ SỐ>" "<TÊN TRẬN>"        "<dòng ghi bàn>" "<pill>" <out.png> [mau_pill]

mau_pill: vang (mặc định) · trang · xam
  · VÔ ĐỊCH → nhãn "VÔ ĐỊCH ASEAN CUP 2026", màu vang
  · Thắng/hoà thường → vang
  · THUA hoặc MẤT CÚP → dùng "xam" hoặc "trang" và nhãn TRUNG TÍNH.
    Không dùng chữ nào nghe như ăn mừng khi đội nhà thua — người đọc đang buồn.
"""
import sys
...
MAU_PILL = {'vang': ((255, 214, 64), (20, 20, 20)),
            'trang': ((238, 240, 238), (20, 20, 20)),
            'xam': ((120, 134, 126), (255, 255, 255))}


def build(cartoon, co_a, co_b, ti_so, ten_tran, line2, pill, out, mau_pill='vang'):
    ...
    c = Image.open(co_a).convert('RGBA').resize((980, 980))
    ...
    # hàng trên: CỜ A  —  TỈ SỐ  —  CỜ B (thay chỗ mũi tên A→B của tin chuyển nhượng)
    for f, x in ((co_a, 230), (co_b, 1090)):
        im = Image.open(f).convert('RGBA')
        im = im.resize((300, int(im.height * 300 / im.width)), Image.LANCZOS)
        bg.paste(im, (x, 282 + (300 - im.height) // 2), im)
    sf = F(150)
    sw = d.textlength(ti_so, font=sf)
    lt(ti_so, 660 + (400 - sw) / 2, 282 + 150 - sf.size // 2, sf, VANG)

    RONG_TEN = 1130
    fn = F(190); wn = d.textlength(ten_tran, font=fn)
    if wn > RONG_TEN:
        fn = F(max(60, int(190 * RONG_TEN / wn)))
        while d.textlength(ten_tran, font=fn) > RONG_TEN and fn.size > 60:
            fn = F(fn.size - 1)
    lt(ten_tran, 230, 762 - fn.size // 2, fn)
    ...
    nen_pill, chu_pill = MAU_PILL.get(mau_pill, MAU_PILL['vang'])
    pf = F(48); pw = d.textlength(pill, font=pf); x0, y0, ph = 230, 986, 96
    if pw + 110 > 1290:                      # nhãn "VÔ ĐỊCH ASEAN CUP 2026" dài hơn nhãn thường
        pf = F(max(30, int(48 * 1290 / (pw + 110))))
        pw = d.textlength(pill, font=pf)
    d.rounded_rectangle((x0+4, y0+6, x0+pw+110+4, y0+ph+6), radius=48, fill=(0, 0, 0, 80))
    d.rounded_rectangle((x0, y0, x0+pw+110, y0+ph), radius=48, fill=nen_pill)
    d.text((x0+55, y0+(ph-pf.size)//2-4), pill, font=pf, fill=chu_pill)
